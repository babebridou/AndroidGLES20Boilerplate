...
public class SimpleUVHandler extends ProgramHandler{

	public static final int FLOAT_SIZE_BYTES = 4;
	
	public SimpleUVHandler(String positionName, int stride, int offsetbytes) {
	...
	public SimpleUVHandler(String positionName, Composition composition) {
		variableName = positionName;
		glLocation = -1;
		switch(composition){
		case FLOATBUFFER_UV:
			type = ProgramHandlerType.glVertexAttribPointer3fv;
			this.attribStride = 2*FLOAT_SIZE_BYTES;
			this.attribOffsetBytes = 0;
			break;
		case FLOATBUFFER_VERTS_UV:
			type = ProgramHandlerType.glVertexAttribPointer3fv;
			this.attribStride = 5*FLOAT_SIZE_BYTES;
			this.attribOffsetBytes = 3*FLOAT_SIZE_BYTES;
			break;
		case FLOATBUFFER_VERTS_NORMAL_UV:
			type = ProgramHandlerType.glVertexAttribPointer3fv;
			this.attribStride = 8*FLOAT_SIZE_BYTES;
			this.attribOffsetBytes = 6*FLOAT_SIZE_BYTES;
			break;
